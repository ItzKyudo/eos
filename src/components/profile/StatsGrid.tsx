...
const StatsGrid: React.FC<StatsGridProps> = ({ user }) => {
  const ratings = [
    { type: 'Classic', rating: user?.rating_classic || 1200, change: user?.rating_classic_change || 0, icon: PIECES.piece7_b },
    { type: 'Swift', rating: user?.rating_swift || 1200, change: user?.rating_swift_change || 0, icon: PIECES.piece3_b },
    { type: 'Turbo', rating: user?.rating_turbo || 1200, change: user?.rating_turbo_change || 0, icon: PIECES.piece5_b },
    { type: 'Rapid', rating: user?.rating_rapid || 1200, change: user?.rating_rapid_change || 0, icon: PIECES.piece1_b },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
        {ratings.map((rating) => {
          const isPositive = rating.change > 0;
          const isNegative = rating.change < 0;
          ...
          return (
            <div key={rating.type} className="bg-[#1e293b] p-5 rounded-xl border border-slate-700 shadow-md hover:shadow-lg hover:border-slate-600 transition group relative overflow-hidden">
              ...
              <div className="text-4xl font-black text-white mb-2 tracking-tight relative z-10 flex items-baseline gap-2">
                {rating.rating}
                {rating.change !== 0 && (
                  <div className={`flex items-center text-sm font-bold ${changeColor} bg-slate-800/50 px-2 py-0.5 rounded-full border border-current opacity-80`}>
                    <span className="mr-1 text-[10px]">{ArrowIcon}</span>
                    <span>{Math.abs(rating.change)}</span>
                  </div>
                )}
              </div>

              <div className={`text-sm font-bold flex items-center gap-1 ${rating.change < 0 ? 'text-red-400/50' : 'text-slate-500'}`}>
                {/* <span>-</span> */}
                <span className="font-normal text-xs ml-0.5">Rank pending</span>
              </div>
            </div>
          );
        })}
